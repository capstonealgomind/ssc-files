...
const form = useForm({
    election_id:   String(props.candidate.election_id),
...
// null  = no new file chosen; show existing photo if available
...
// 2. Existing stored photo (props.candidate.photo_url)
const displayPhoto = computed(() => photoPreview.value || props.candidate.photo_url || null);
...
function submit() {
    form
        ...
        .post(`/candidates/${props.candidate.id}`, {
            forceFormData: true,
            onError: () => {
                toastError(
                    'Action failed',
...
<template>
    <AppLayout>
        ...
        <template #header>
            <h1 class="text-base font-semibold" style="color: hsl(240 10% 3.9%);">Edit Candidate</h1>
        </template>

        <div class="w-full space-y-5">
            <button
                type="button"
                class="inline-flex items-center gap-1.5 text-sm transition-colors hover:underline"
                style="color: hsl(240 3.8% 46.1%);"
                @click="router.visit('/candidates')"
            >
            ...
            <form @submit.prevent="submit">
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm mt-0.5" style="color: hsl(240 3.8% 46.1%);">
                                    Update the basic information for this candidate.
                                </p>
                            </div>
                    ...
                    <div class="space-y-5">
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm mt-0.5" style="color: hsl(240 3.8% 46.1%);">
                                    Replace or keep the existing photo. JPG, PNG or WebP · max 2 MB.
                                </p>
                            </div>
                            ...
                            >
                                <div v-if="displayPhoto" class="flex flex-col items-center gap-3 py-6 px-4">
                                    <img
                                        :src="displayPhoto"
                                        alt="Candidate photo"
                                        class="h-36 w-36 rounded-full object-cover"
                                        style="box-shadow: 0 0 0 4px hsl(240 5.9% 90%);"
                                    />
                ...
                <div class="mt-5 flex items-center justify-end gap-3">
                    <Button type="button" variant="outline" @click="router.visit('/candidates')">Cancel</Button>
